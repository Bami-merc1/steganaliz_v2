...
export function validateExtension(file: File): ValidationResult {
  ...
  if (!SUPPORTED_CARRIER_EXTENSIONS.includes(ext)) {
    return { valid: false, reason: `Extension ".${ext}" is not a supported carrier format.` };
  }
  return { valid: true };
}
...
export async function validateMagicNumber(file: File): Promise<ValidationResult> {
  ...
  if (!candidates) return { valid: true }; // no signature check for this ext
  if (candidates.length === 0) {
    // Special case: mp4 validated via ftyp box
    if (ext === 'mp4') {
...
export async function validateZipExpansion(file: File): Promise<ValidationResult> {
  const ext = file.name.split('.').pop()?.toLowerCase() ?? '';
  const zipExtensions = ['zip', 'docx', 'pptx', 'xlsx', 'odt', 'odx', 'apk'];
  ...
  try {
    ...
    while (offset < bytes.length - 30) {
      if (
        ...
        if (totalUncompressed > 0 && file.size > 0 &&
            totalUncompressed / file.size > MAX_ZIP_EXPANSION_RATIO) {
          return {
            valid: false,
            reason: `Suspicious compression ratio (${Math.round(totalUncompressed / file.size)}×) — possible zip bomb. File rejected.`,
          };
        }
  ...
  } catch { /* malformed ZIP — let the engine handle it */ }
  return { valid: true };
}

export async function validateCarrierFile(file: File): Promise<ValidationResult> {
  const syncChecks = [validateFileSize(file), validateExtension(file), validateMimeType(file)];
